...
import type { ArchiveBase, ArchiveDialogRelatedDuplex, ArchiveDuplex, RelatedEntry } from "@/modules/archives/types.ts";
...
export const useArchiveDuplex = ({ open, archiveId }: ArchiveDuplex) => {
    ...
    const [paginationViewDuplex, setPaginationViewDuplex] = useState<Pagination>()
    const [filtersDuplex, setFiltersDuplex] = useState<ArchiveDialogRelatedDuplex>(getFiltersFromURL)
    ...
    useEffect(() => {
        ...
        getArchiveById()
    }, [archiveId, filtersDuplex, handleApiError, navigate, open, r_limit, r_page])
